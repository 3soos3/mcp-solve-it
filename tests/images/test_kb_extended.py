...
class TestMitigationsForWeakness:
    def test_dfw_1002_mitigations_non_empty(
        self, bundled_client: PodmanMCPClient
    ) -> None:
...
class TestMitigationsForTechnique:
    def test_dft_1001_mitigations_non_empty(
        self, bundled_client: PodmanMCPClient
    ) -> None:
        data = bundled_client.call_tool(
        ...
        payload = bundled_client.unwrap(data)
        count = (
            len(payload.get("mitigations", [])) if isinstance(payload, dict)
            else len(payload) if isinstance(payload, list)
            else 0
        )
...
class TestGetMitigation:
    def test_dfm_1001_returns_name(self, bundled_client: PodmanMCPClient) -> None:
        ...
        payload = bundled_client.unwrap(data)
        assert isinstance(payload, dict) and "name" in payload, \
            f"DFM-1001 missing name: {payload}"
...
class TestTechniquesForWeakness:
    ...
    def test_reverse_relationship_consistent(self, bundled_client: PodmanMCPClient) -> None:
        ...
        assert "DFT-1001" in ids, \
            f"DFT-1001 not found in reverse lookup for DFW-1001: {ids[:5]}"
...
class TestCitations:
    ...
    def test_citation_count_consistent_with_status(
        self, bundled_client: PodmanMCPClient
    ) -> None:
        _require_tool(bundled_client, "solveit_list_citations")
        status = bundled_client.call_tool("solveit_status")
...
class TestExtensions:
    def test_list_loaded_extensions_returns_data(
        self, bundled_client: PodmanMCPClient
    ) -> None:
...
class TestSmokeAllTools:
    ...
    def test_all_tools_respond_without_crash(
        self, bundled_client: PodmanMCPClient
    ) -> None:
